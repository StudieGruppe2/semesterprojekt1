...
async function hentPlaylist() {
  ...
  // Sorterer afspillede sange bagerst
  const ikkAfspillede = sange.filter(function (sang) {
    return !afspilledeSange.includes(sang.track_id);
  });
  const alleredeAfspillede = sange.filter(function (sang) {
    return afspilledeSange.includes(sang.track_id);
  });
  const sorteretSange = ikkAfspillede.concat(alleredeAfspillede);

  if (sorteretSange.length > 0 && !timerStartet) {
    timerStartet = true;
    afspilledeSange.push(sorteretSange[0].track_id);

    // Nulstil hvis alle sange er spillet
    if (ikkAfspillede.length === 0) {
      afspilledeSange = [sorteretSange[0].track_id];
    }

    startTimer(sorteretSange[0].duration_ms, sorteretSange, 0);
  }
